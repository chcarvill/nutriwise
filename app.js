...
function addFood(catId)                      { foods[catId].push({ name:"", freq:"", days:"", score:"", portion:"" }); openCats[catId] = true; renderFoods(); saveToStorage(); }
...
function emailFoodsData() {
  ...
  CATEGORIES.forEach(cat => {
    ...
    items.forEach(f => {
      ...
      lines.push(`  ${(f.name || "").padEnd(28)} ${freq.padEnd(10)} ${days.padEnd(10)} ${score.padEnd(14)} ${portion}`);
    });
    lines.push("");
  });
  lines.push(
    "COLUMNS: Food name | Times per day | Days per week | Experience score | My portion",
    "", "-".repeat(40),
    "Generated by NutriWise — for informational purposes only.",
    "Consult a qualified dietitian for personalised medical nutrition advice.",
  );
